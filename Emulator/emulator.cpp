...
int parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
	...
	if ( pscnt > 0 ) {
		...
		switch( op ) {
			...
			//instruction added
			case ANDN: case CLMUL: case CLMULH: case CLMULR: case MAX: case MAXU: case MIN: case MINU: case ORN: case ROL: case ROR:
			    if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			    i->a1.reg = parse_reg(o1 , line);
			    i->a2.reg = parse_reg(o2 , line);
			    i->a3.reg = parse_reg(o3 , line);
			...
			case RORI:
				...
				return 1;
			// instrction rd, rs1, rs2
			case BCLR: case BEXT: case BINV: case BSET: case SH1ADD: case SH2ADD: case SH3ADD: case XNOR:
				if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line, "Invalid format" );
				i->a1.reg = parse_reg(o1, line);
				i->a2.reg = parse_reg(o2, line);
				i->a3.reg = parse_reg(o3, line);
				return 1;
			// instrction rd, rs1, shamt
			case BCLRI: case BEXTI: case BINVI: case BSETI:
				if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line, "Invalid format" );
				i->a1.reg = parse_reg(o1, line);
				i->a2.reg = parse_reg(o2, line);
				i->a3.imm = parse_imm(o3, 5, line);
...
void execute(uint8_t* mem, instr* imem, label_loc* labels, int label_count, bool start_immediate) {
	...
	while(!dexit) {
		...
		switch (i.op) {
			...
			case CLMUL:
				for(int ins_idx = 0; ins_idx < 32; ins_idx++)
					rf[i.a1.reg] = ((rf[i.a3.reg] >> ins_idx) & 1)? rf[i.a1.reg] ^ (rf[i.a2.reg] << ins_idx): rf[i.a1.reg];
				break;
			// clmulh: Carry-less multiply (high-part)
			case CLMULH:
				for(int ins_idx = 1; ins_idx < 33; ins_idx++)
					rf[i.a1.reg] = ((rf[i.a3.reg] >> ins_idx) & 1)? rf[i.a1.reg] ^ (rf[i.a2.reg] >> (32 - ins_idx)): rf[i.a1.reg];
				break;
			// clmulr: Carry-less multiply (reversed)
			case CLMULR:
				for(int ins_idx = 0; ins_idx < 32; ins_idx++)
					rf[i.a1.reg] = ((rf[i.a3.reg] >> ins_idx) & 1)? rf[i.a1.reg] ^ (rf[i.a2.reg] >> (31 - ins_idx)): rf[i.a1.reg];
				break;
			// clz: Count leading zero bits
			case CLZ:
				rf[i.a1.reg] = 0;
				for(int ins_idx = 0; ins_idx < 32; ins_idx++){
					if(((rf[i.a2.reg] >> (31 - ins_idx)) & 1) == 0){
						rf[i.a1.reg] = ins_idx + 1;
					}else{
						break;
					}
				}
				break;
			// cpop: Count set(1) bits
			case CPOP:
				rf[i.a1.reg] = 0;
				for(int ins_idx = 0; ins_idx < 32; ins_idx++){
					if((rf[i.a2.reg] >> ins_idx) & 1)
						rf[i.a1.reg]++;
				}
				break;
			...
			case CTZ:
				...
				break;
			// max: Maximum (Register)
			case MAX: rf[i.a1.reg] = ((int)rf[i.a2.reg] > (int)rf[i.a3.reg]) ? rf[i.a2.reg] : rf[i.a3.reg]; break;
			// maxu: Unsigned maximum (Register)
			case MAXU: rf[i.a1.reg] = (rf[i.a2.reg] > rf[i.a3.reg]) ? rf[i.a2.reg] : rf[i.a3.reg]; break;
			// min: Minimum (Regitser)
			case MIN: rf[i.a1.reg] = ((int)rf[i.a2.reg] < (int)rf[i.a3.reg]) ? rf[i.a2.reg] : rf[i.a3.reg]; break;
			// minu: Unsigned Minimum (Regitser)
			case MINU: rf[i.a1.reg] = (rf[i.a2.reg] < rf[i.a3.reg]) ? rf[i.a2.reg] : rf[i.a3.reg]; break;
			// orc.b: Bitwise OR-Combine, byte granule (Regitser)
			case ORC_B:
				rf[i.a1.reg] = (((rf[i.a2.reg] & (0xFF << 24)) ? 0xFF : 0) << 24) |
							   (((rf[i.a2.reg] & (0xFF << 16)) ? 0xFF : 0) << 16) |
							   (((rf[i.a2.reg] & (0xFF <<  8)) ? 0xFF : 0) <<  8) |
							   (((rf[i.a2.reg] & (0xFF <<  0)) ? 0xFF : 0) <<  0);
				break;
			// orn: OR with inverted operand (Register)
			case ORN: rf[i.a1.reg] = rf[i.a2.reg] | ~rf[i.a3.reg]; break;
			// rev8: Byte-reverse register (Register)
			case REV8:
				rf[i.a1.reg] = (((rf[i.a2.reg] >>  0) & 0xFF) << 24) |
							   (((rf[i.a2.reg] >>  8) & 0xFF) << 16) |
							   (((rf[i.a2.reg] >> 16) & 0xFF) <<  8) |
							   (((rf[i.a2.reg] >> 24) & 0xFF) <<  0);
				break;
			...
			case RORI: rf[i.a1.reg] = (rf[i.a2.reg] >> i.a3.imm) | (rf[i.a2.reg] << (32 - i.a3.imm)); break;
			// bclr: Single-Bit Clear (Register)
			case BCLR: rf[i.a1.reg] = rf[i.a2.reg] & ~(1 << (rf[i.a3.reg] & 31)); break;
			// bclri: Single-Bit Clear (Immediate)
			case BCLRI: rf[i.a1.reg] = rf[i.a2.reg] & ~(1 << (i.a3.imm & 31)); break;
			// bext: Single-Bit Extract (Register)
			case BEXT: rf[i.a1.reg] = (rf[i.a2.reg] >> (rf[i.a3.reg] & 31)) & 1; break;
			// bexti: Single-Bit Extract (Immediate)
			case BEXTI: rf[i.a1.reg] = (rf[i.a2.reg] >> (i.a3.imm & 31)) & 1; break;
			// binv: Single-Bit Invert (Register)
			case BINV: rf[i.a1.reg] = rf[i.a2.reg] ^ (1 << (rf[i.a3.reg] & 31)); break;
			// binvi: Single-Bit Invert (Immediate)
			case BINVI: rf[i.a1.reg] = rf[i.a2.reg] ^ (1 << (i.a3.imm & 31)); break;
			// bset: Single-Bit Set (Register)
			case BSET: rf[i.a1.reg] = rf[i.a2.reg] | (1 << (rf[i.a3.reg] & 31)); break;
			
			// bseti: Single-Bit Set (Immediate)
			case BSETI: rf[i.a1.reg] = rf[i.a2.reg] | (1 << (i.a3.imm & 31)); break;
			// sext.b: Sign-extend byte
			case SEXT_B: rf[i.a1.reg] = signextend(rf[i.a2.reg],8); break;
			// sext.h: Sign-extend halfword
			case SEXT_H: rf[i.a1.reg] = signextend(rf[i.a2.reg],16); break;
			// sh1add: Shift left by 1 and add
			case SH1ADD: rf[i.a1.reg] = rf[i.a3.reg] + (rf[i.a2.reg] << 1); break;
			// sh2add: Shift left by 2 and add
			case SH2ADD: rf[i.a1.reg] = rf[i.a3.reg] + (rf[i.a2.reg] << 2); break;
